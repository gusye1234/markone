    ...
    Object.assign(
      {
        ...
        titleBarStyle: 'hidden' as MakeTSShutUp,
  ...
  if (CONFIG.get('settings.enableGlobalHotkey')) {
    try {
      const ret = globalShortcut.register(
        CONFIG.get('settings.globalHotkey'),
        () => {
          if (!win) {
            return;
          }
...
ipcMain.handle('settings:set', (event, settings) => {
  ...
  let globalHotkeyChanged =
    settings.enableGlobalHotkey !== CONFIG.get('settings.enableGlobalHotkey') ||
    settings.globalHotkey !== CONFIG.get('settings.globalHotkey');
